...
func (s *SliceStackInfo) Marshal3MFAttr(_ *go3mf.XMLEncoder) ([]xml.Attr, error) {
	return []xml.Attr{
		{Name: xml.Name{Space: ExtensionSpace, Local: attrSliceRefID}, Value: strconv.FormatUint(uint64(s.SliceStackID), 10)},
		{Name: xml.Name{Space: ExtensionSpace, Local: attrMeshRes}, Value: s.SliceResolution.String()},
	}, nil
}

// Marshal3MF encodes the resource.
func (s *SliceStack) Marshal3MF(x *go3mf.XMLEncoder) error {
	xs := xml.StartElement{Name: xml.Name{Space: ExtensionSpace, Local: attrSliceStack}, Attr: []xml.Attr{
	...
	for _, r := range s.Refs {
		x.EncodeToken(xml.StartElement{Name: xml.Name{Space: ExtensionSpace, Local: attrSliceRef}, Attr: []xml.Attr{
...
func (s *Slice) marshal3MF(x *go3mf.XMLEncoder) {
	xs := xml.StartElement{Name: xml.Name{Space: ExtensionSpace, Local: attrSlice}, Attr: []xml.Attr{
...
func marshalPolygons(x *go3mf.XMLEncoder, ply []Polygon) {
	for _, p := range ply {
		xp := xml.StartElement{Name: xml.Name{Space: ExtensionSpace, Local: attrPolygon}, Attr: []xml.Attr{
			{Name: xml.Name{Local: attrStartV}, Value: strconv.FormatUint(uint64(p.StartV), 10)},
		}}
		x.EncodeToken(xp)
		x.SetAutoClose(true)
		for _, s := range p.Segments {
			xs := xml.StartElement{Name: xml.Name{Space: ExtensionSpace, Local: attrSegment}, Attr: []xml.Attr{
...
func marshalVertices(x *go3mf.XMLEncoder, vs []go3mf.Point2D) {
	xv := xml.StartElement{Name: xml.Name{Space: ExtensionSpace, Local: attrVertices}}
	x.EncodeToken(xv)
	x.SetAutoClose(true)
	for _, v := range vs {
		x.EncodeToken(xml.StartElement{Name: xml.Name{Space: ExtensionSpace, Local: attrVertex}, Attr: []xml.Attr{
